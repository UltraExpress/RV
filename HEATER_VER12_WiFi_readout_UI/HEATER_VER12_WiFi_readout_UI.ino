...
#define BUTTON_RELEASE_GRACE_MS 200  // Grace period for releasing buttons
...
const char* www_password = "RVthermostat";
...
if (isAP && setupMode) {  // Only show scrolling message in setup mode
...
void showSetupScreen() {
  ...
  display.setCursor(0, 16);
  display.println("WiFi: RV-THERMOSTAT");
  
  display.setCursor(0, 26);
  display.println("Pass: RVthermostat");
...
void enterSetupMode(String reason) {
  ...
  setupMode = true;
  isAP = true;
  displaySleepMode = false;  // Ensure display is on
  
  WiFi.disconnect();
  WiFi.mode(WIFI_AP);
  if (WiFi.softAP("RV-THERMOSTAT", "RVthermostat")) {
    Serial.println("AP Started Successfully");
    
    // Show setup screen first and make it stick
    showSetupScreen();
    wifiMessage = "SETUP MODE - Connect to: RV-THERMOSTAT (RVthermostat) -> 192.168.4.1";
    
    // Simple, flat route setup
    server.on("/", handleWiFiSetup);
    server.on("/control", handleRoot);  // Direct to main control interface
...
if (!server.authenticate(www_username, www_password)) {
    ...
    server.sendHeader("Cache-Control", "post-check=0,pre-check=0");
    server.sendHeader("Pragma", "no-cache");
    server.sendHeader("Expires", "-1");
    String authPage = "<!DOCTYPE html><html><head>";
    authPage += "<meta name='viewport' content='width=device-width,initial-scale=1'>";
    authPage += "<meta http-equiv='refresh' content='0'>";  // Force refresh
    authPage += "<style>body{font-family:-apple-system,sans-serif;text-align:center;padding:40px;}</style>";
    authPage += "</head><body><h2>Authentication Required</h2>";
    authPage += "<script>if(navigator.userAgent.includes('Safari') && !navigator.userAgent.includes('Chrome')){window.location.reload(true);}</script>";
    authPage += "</body></html>";
    server.send(401, "text/html", authPage);
    return;
}
...
html += ".wifi-dots{display:flex;gap:2px;}";
html += ".wifi-dot{width:6px;height:6px;border-radius:50%;transition:background-color 0.3s;}";
...
//web wifi dots colors
html += "function updateWifiDots(rssi){";
html += "  const dots=[1,2,3,4,5];";
html += "  let level = 0;";
html += "  if(rssi > -50) level = 5;";
html += "  else if(rssi > -60) level = 4;";
html += "  else if(rssi > -70) level = 3;";
html += "  else if(rssi > -80) level = 2;";
html += "  else level = 1;";
html += "  dots.forEach(i=>{";
html += "    const dot=document.getElementById('wifi'+i);";
html += "    if(i<=level){";
html += "      const color=i===1?'#FF3B30':i===2?'#FF9500':i===3?'#FFCC00':i===4?'#99CC00':'#34C759';";
html += "      dot.style.background=color;";
html += "    }else{";
html += "      dot.style.background='#ddd';";
html += "    }";
html += "  });";
html += "}";
...
html += "updateWifiDots(d.rssi);";html += "document.getElementById('h').style.background = d.heaterPinState ? '#FF3B30' : '#007AFF';";
html += "document.getElementById('heatStatus').textContent = d.heaterPinState ? 'Heat ON' : 'Heat OFF';";
...
html += "<div class='s' style='justify-content:space-between;border-bottom:1px solid #ddd;padding-bottom:8px;margin-bottom:12px'>";
html += "<div style='display:flex;align-items:center;gap:5px'>";  // Left group
html += "<div id='d' class='dot' style='background:red'></div>";
html += "<span id='status' style='font-size:12px'>Connecting...</span>";
html += "</div>";

html += "<div style='display:flex;align-items:center;gap:5px'>";  // Center group wrapper
html += "<div class='wifi-dots'>";  // Dots container
html += "<div class='wifi-dot' id='wifi1'></div>";
html += "<div class='wifi-dot' id='wifi2'></div>";
html += "<div class='wifi-dot' id='wifi3'></div>";
html += "<div class='wifi-dot' id='wifi4'></div>";
html += "<div class='wifi-dot' id='wifi5'></div>";
html += "</div>";
html += "<span style='font-size:12px'>WiFi</span>";  // Added WiFi text
html += "</div>";


html += "<div style='display:flex;align-items:center;gap:5px'>";  // Right group
html += "<div id='h' class='dot' style='background:blue'></div>";
html += "<span id='heatStatus' style='font-size:12px'>Heat OFF</span>";
html += "</div>";
html += "</div>";
...
if (server.hasArg("direct")) {
    ...
    handleRoot();         // Show control interface
    return;
}
...
html += "<a href='/?direct=1' class='link-btn direct-btn' style='margin-top:12px;display:block;text-align:center;padding:12px;background:#34C759;color:white;border-radius:8px;text-decoration:none;'>Direct Control Mode</a>";    server.send(200,"text/html",html);
  }
}
...
void handleReset() {
  ...
  html += "<style>body{font-family:-apple-system,sans-serif;text-align:center;padding:20px;} .info{margin:20px;padding:15px;border:1px solid #ccc;border-radius:8px;}</style>";
  html += "</head><body>";
  html += "<h2>Device is resetting...</h2>";
  html += "<p>Restarting in setup mode. Please connect using:</p>";
  html += "<div class='info'>";
  html += "<p><strong>WiFi Network:</strong> RV-THERMOSTAT<br>";
  html += "<strong>Network Password:</strong> RVthermostat</p>";
  html += "<p><strong>Web Interface:</strong> 192.168.4.1<br>";
  html += "<strong>Username:</strong> admin<br>";
  html += "<strong>Password:</strong> RVthermostat</p>";
  html += "</div></body></html>";
  server.send(200,"text/html",html);
...
void handleData() {
  ...
  // Read actual heater pin state
  bool actualHeaterState = digitalRead(HEATER_PIN);
  ...
  json += "\"heaterEnabled\":";
  json += (heaterEnabled ? "true" : "false");  // For toggle state
  json += ",";
  json += "\"heaterPinState\":";
  json += (actualHeaterState ? "true" : "false");  // For indicator dot
  json += ",";
...
json += ",\"rssi\":";
json += String(WiFi.RSSI());  // Add RSSI value
json += "}";
...
void handleButtons() {
    ...
    static bool buttonsWereReleased = true;

    if (displaySleepMode) {
        // Only allow wake if buttons were previously released and grace period passed
        if ((now - sleepStartTime > 500) && 
            buttonsWereReleased && 
            (currentUpState == LOW || currentDownState == LOW)) {
            displaySleepMode = false;
            display.dim(false);
            showingStateChange = true;
        ...
        // Track if buttons are released while in sleep mode
        buttonsWereReleased = (currentUpState == HIGH && currentDownState == HIGH);
        return; // Don't process other button actions while sleeping
    }
    ...
    if (!displaySleepMode) {
        ...
        // DOWN long press check - Freeze Guard at 1s, WiFi Reset at 5s
// DOWN long press check - Freeze Guard at 1s, WiFi Reset at 5s
        if (currentDownState == LOW) {
            if (now - downButtonStart >= WIFI_RESET_MS) {  // 5 second hold
                showingStateChange = true;
            ...
            else if (now - downButtonStart >= LONG_PRESS_MS && !longPressHandled) {  // 1 second hold
